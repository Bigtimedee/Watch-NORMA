...
- `lib/__tests__/auth-recovery-landing.test.ts` — web recovery never lands on marketing `/`; hosted site_url + allowlist + expired-link routing
...
- **Version on `main` (2026-09-04):** `app.json` is **1.5.0 / iOS build 23**. Marketing copy that claimed 1.6.0 / build 33 “Waiting for Review” is not reflected in this repository. `eas.json` has `cli.appVersionSource: remote` and production `autoIncrement: true`, so the App Store Connect build number can drift ahead of `app.json` — treat ASC as the live binary source of truth, not git. Native scheme additions (`prizepicks`, `underdog` already in 1.5.0 source; `sleeper` / `yahoosports` added on this branch) require a new EAS iOS build and App Store submission; JS-only changes (roster import, brand colors, pick'em screen) can ship via EAS OTA after merge to `main`. Do not bump `app.json` version unless a native binary ship is intentionally cut.
- **DFS / pick'em backend deploy (required after merge):** apply `092_prizepicks_underdog_dfs_pickem.sql` if not already on production (apply status **unknown** as of 2026-09-04 — Supabase MCP was unauthenticated in the audit session; verify with `to_regclass` / `streaming_providers` rows, not the migration ledger — see `12_PRODUCTION_RECONCILIATION_2026_07.md`). Then apply `20260904183000_dfs_fantasy_integration_fixes.sql` (`follows.fantasy_source`, unique upsert constraint, season-long providers, pick'em `sportsbook_restrictions`). Redeploy `evaluate-alerts` (and any function that bundles `_shared/auction-engine.ts` / `email-parser.ts` / `sportsbook-links.ts`). Code-on-main ≠ App Store live ≠ production schema.
...
- **Public marketing proof:** Home, `/advertisers`, `/demo`, `/developers`, and the public OpenAPI description must not render fabricated delivery zeros (`0% CTR`, `$0.0`) or unpublished CTR ranges as proof. `StatCounter` vanity blocks were removed until real volume exists.
...
6. **Apply Auth Redirects job** (added 2026-09-19, main branch push only): `node scripts/apply-auth-redirects.mjs` PATCHes hosted `site_url` + `uri_allow_list` from `supabase/auth-redirects.json`. Same secrets. **Site URL must be `https://getnorma.app/auth/reset-password`, never marketing `/`.** Missing secrets warn and skip (dashboard URL Configuration is the fallback). Dispatch-only re-apply: `.github/workflows/apply-auth-redirects.yml`.
7. **OTA Update job:** (main branch push only) `eas update --auto --channel production --non-interactive`. Now depends on `deploy-functions` so the client bundle never ships to devices before the backend it depends on.
