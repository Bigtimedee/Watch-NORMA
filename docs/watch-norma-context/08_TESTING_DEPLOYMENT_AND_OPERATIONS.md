...
5. **Apply Auth Email Templates job** (added 2026-09-19, main branch push only): `node scripts/apply-auth-email-templates.mjs` PATCHes hosted GoTrue mailer subjects/bodies from `supabase/templates/`. Gated on the same `SUPABASE_ACCESS_TOKEN` / `SUPABASE_PROJECT_REF` secrets as function deploy. Missing secrets warn and skip (dashboard paste is the fallback). First main run after PR #43: secrets present, Management API **401** (same token failure as Deploy Edge Functions / #42 main CI). Rotate the PAT and dispatch **Apply Auth email templates**, or paste `supabase/templates/` in the dashboard. Dispatch-only re-apply: `.github/workflows/apply-auth-email-templates.yml`.
