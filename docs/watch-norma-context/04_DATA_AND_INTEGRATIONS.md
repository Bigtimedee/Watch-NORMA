...
| Betr (Betr Picks) | DFS pick'em — third operator after PP/UD | **Phase A shipped** (registry, geo, brands, Connections, OneLink). Email/slip/auction sport-board still later. | `lib/fantasy-platforms.ts`, `lib/sportsbook-brands.ts`, `_shared/sportsbook-links.ts`, `app/(tabs)/connections/{index,pickem}.tsx`, migration `20260914210000_betr_dfs_pickem.sql`. Plan: [`docs/betr-integration-plan.md`](../betr-integration-plan.md). | v1 key = **`betr`**. Display "Betr" / picker "Betr Picks". Not BetRivers. No public consumer API. `ios_scheme` is NULL (OneLink `betr.onelink.me/VZxy/betrapp`). CTA = **Open Betr**. `sportsbook_restrictions` from help-center Picks=Yes (includes TN; Play Store omits TN — documented in the migration). SSBK is a later/optional sportsbook track with a different key. |
