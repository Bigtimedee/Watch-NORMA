...
| X/Twitter API | Social publishing | **Implemented** | `cmo-publish`, `publish-social-posts`, `fetch-social-metrics` | OAuth 1.0a, v2 API + v1.1 media upload. `cmo-publish` is twitter-only on `content_calendar` (`platform = 'twitter'`); it must not tweet LinkedIn/Instagram/TikTok/Facebook drafts. Preflight revalidate immediately before `postTweet` skips `paused`/non-draft rows (incident `310441ec`, 2026-09-15); drafts still auto-publish when due. |
| LinkedIn Posts / UGC API | Company-page publishing | **Implemented** | `cmo-publish-linkedin` | Organization author only (`urn:li:organization:146336141`). Company page: https://www.linkedin.com/company/watch-norma/. Reads `content_calendar` where `platform = 'linkedin'`. Requires `LINKEDIN_ACCESS_TOKEN`. `LINKEDIN_ORGANIZATION_ID` is optional (env → `social_accounts.account_id` where platform=linkedin → hardcoded `146336141`). Text + first `media_urls` image. Not wired through `publish-social-posts` / `social_posts`. Provision via `.github/workflows/set-linkedin-secrets.yml` (manual dispatch; org id secret is optional because of the in-code default). |
...
| Betr (Betr Picks) | DFS pick'em — third operator after PP/UD | **Tier B/C** (connect, roster paste, slip scan, sponsor CTA). Email ingest **blocked on fixture**. | `lib/fantasy-platforms.ts`, `lib/sportsbook-brands.ts`, `_shared/sportsbook-links.ts`, `parse-bet-slip`, `email-parser.ts` (gated), `ImportRosterSheet`, `app/(tabs)/connections/{index,pickem}.tsx`, `creative-prescreen/rubric.ts` (`flagPickEmBetNowCopy`), migration `20260914210000_betr_dfs_pickem.sql`. Plan: [`docs/betr-integration-plan.md`](../betr-integration-plan.md). | v1 key = **`betr`**. Display "Betr" / picker "Betr Picks". Not BetRivers — never match a bare `betr` token. No public consumer API. `ios_scheme` is NULL (OneLink `betr.onelink.me/VZxy/betrapp`). CTA = **Open Betr** (never "Bet Now on Betr"). Auction CTAs rewrite `betr.app` / `picks.betr.app` / `betr.onelink.me` to the OneLink (no fake sport board). `sportsbook_restrictions` from help-center Picks=Yes (includes TN). Slip OCR enum includes `betr`. Email domain unmapped until a redacted confirmation fixture exists. SSBK is a later/optional sportsbook track with a different key. |
