...
| Betr (Betr Picks) | DFS pick'em — third operator after PP/UD | **Planned — not implemented** | Plan: [`docs/betr-integration-plan.md`](../betr-integration-plan.md). Mirror PrizePicks/Underdog (`FANTASY_PLATFORMS`, `PICKEM_PROVIDER_KEYS`, `dfs_pickem`, `useSportsbookGeo`). | v1 = **Betr Picks** only (`provider_key = betr`). Not BetRivers. No public consumer API found. SSBK is a later/optional sportsbook track with a different key and state list. Seed `sportsbook_restrictions` from cited Betr sources (help-center matrix + store listings); do not copy the PrizePicks/Underdog arrays. |
