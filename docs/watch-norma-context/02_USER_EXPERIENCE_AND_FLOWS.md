...
**Step 4b: Import a fantasy roster.** From the Connections (Watch) tab, `ImportRosterSheet` accepts a pasted multi-line blob of player names. The user picks a platform from `FANTASY_PLATFORMS` (`lib/fantasy-platforms.ts`: DraftKings DFS, Yahoo Fantasy, Sleeper, ESPN Fantasy, PrizePicks, Underdog, Betr Picks, Other). `parseRosterInput()` and `buildRosterFollowRows()` in `lib/roster-import.ts` normalise each name and write `follows` rows with `follow_type: "player"`, `source: "fantasy"`, and `fantasy_source` set to the selected platform. `evaluate-alerts` includes those users as candidates when a followed player appears in the game's Sportradar summary or ESPN boxscore. The connections screen surfaces a count of these fantasy-sourced follows separately from manual ones. Pick'em apps (PrizePicks, Underdog, Betr) are also connectable (Tier C "I use this") on Connections → Pick'em & DFS.
