...
- **CI/CD secrets:** `EXPO_TOKEN` is stored as a GitHub Actions secret for OTA updates. `SUPABASE_ACCESS_TOKEN` and `SUPABASE_PROJECT_REF` are GitHub Actions secrets used by Edge Function deploy and the dispatch-only LinkedIn secrets workflow. Optional future owner auth health-check should read `OWNER_AUTH_PASSWORD` from GitHub Actions secrets only — document the name, do not commit a vault file or a real password.
...
- **LinkedIn company page:** `LINKEDIN_ACCESS_TOKEN` is a required Supabase secret used by `cmo-publish-linkedin` (never hardcoded; optional `workflow_dispatch` input on `.github/workflows/set-linkedin-secrets.yml` only). `LINKEDIN_ORGANIZATION_ID` is optional and defaults to the public NORMA org id `146336141` (`urn:li:organization:146336141`, https://www.linkedin.com/company/watch-norma/). Optional `LINKEDIN_CLIENT_ID` / `LINKEDIN_CLIENT_SECRET` / `LINKEDIN_REFRESH_TOKEN` refresh the access token on 401. Never log token values. The organization id must be a company page, never a person URN.
