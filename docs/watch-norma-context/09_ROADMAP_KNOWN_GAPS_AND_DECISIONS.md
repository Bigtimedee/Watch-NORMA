...
- ~~**`mcp.getnorma.app` DNS not yet configured.**~~ **Closed (June 2026).** OAuth client created ("NORMA MCP Server") at `getnorma.app/settings`. MCP server deployed to Railway (`watch-norma-production.up.railway.app`) with `NORMA_API_KEY`, `NORMA_OAUTH_CLIENT_ID`, and `NORMA_OAUTH_CLIENT_SECRET`. CNAME `mcp.getnorma.app → watch-norma-production.up.railway.app` added at name.com and confirmed propagated. `api-client.ts` handles OAuth token exchange automatically.
