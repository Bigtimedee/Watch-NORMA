...
0b. ~~**Password reset link dumped users on the marketing homepage.**~~ **Closed (2026-09-19).** After #43 the CTA was clickable, but verify still landed on `https://getnorma.app/?error=access_denied&error_code=otp_expired` (no set-password form). Cause: hosted `site_url` was marketing `/`; `www.getnorma.app` was not allowlisted so `redirectTo` was rejected. Fix: `site_url` = `https://getnorma.app/auth/reset-password`; allowlist apex+www reset/callback paths; homepage interceptor + expired UX; `docs/operations/auth-redirects.md`. Mobile `norma://auth-callback` unchanged.
