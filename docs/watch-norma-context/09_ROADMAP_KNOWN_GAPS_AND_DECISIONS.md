...
- **DFS / fantasy has no live API.** PrizePicks, Underdog, Sleeper, Yahoo Fantasy, ESPN Fantasy, and DraftKings DFS are Tier B/C only (paste import, slip scan, email parse). No public consumer roster API exists — do not invent one. `buildPickEmLink` is wired through `contextualizeSponsorCtaUrl` in both auction-engine return paths so a PrizePicks/Underdog creative CTA is rewritten to a sport-scoped board with campaign attribution. `sportsbook_restrictions` now has pick'em rows (migration `20260904183000`; Player Picks for PrizePicks, classic Pick'em OR Champions for Underdog). `useSportsbookGeo` remains fail-closed. Player-follow alerts require a Sportradar summary or ESPN boxscore player list for the game; if neither snapshot has names, imported rosters do not become candidates. Season-long apps (Sleeper / Yahoo / ESPN Fantasy) are roster-import + deep-link metadata only.
