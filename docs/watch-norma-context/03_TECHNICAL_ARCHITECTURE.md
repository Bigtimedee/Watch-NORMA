...
**Routing:** Expo Router 6 (file-based). Routes map to filesystem under `app/`. Auth routes under `(auth)/`, authenticated routes under `(tabs)/`. Deep link scheme: `norma://`. Mobile auth recovery / magic-link emails use `norma://auth-callback` (`redirectTo` from `hooks/useAuth.ts`; local `supabase/config.toml` `site_url`). `AuthGate` leaves `/auth-callback` and `/(auth)/reset-password` alone during `PASSWORD_RECOVERY` so a recovery session is not bounced to Games before the user sets a password.

**Auth emails:** Bodies are Supabase Auth mailer templates, not a custom NORMA sender. In-repo HTML lives in `supabase/templates/` and is wired for local CLI via `[auth.email.template.*]` in `config.toml`. Hosted production (`shijrazlzawjpobrpmnt`) is updated with `scripts/apply-auth-email-templates.mjs` (Management API) or a dashboard paste — see `docs/operations/auth-email-templates.md`. Every link-based template must use `<a href="{{ .ConfirmationURL }}">` plus a raw URL line.

**Auth URL config (web admin recovery):** Hosted `site_url` must be `https://getnorma.app/auth/reset-password`, never marketing `/`. Allowlist (apex + www + mobile) lives in `supabase/auth-redirects.json` and is applied with `scripts/apply-auth-redirects.mjs`. The web forgot-password flow calls `resetPasswordForEmail` with same-origin `redirectTo` `/auth/reset-password` (no query string). `/?error=` / `/?code=` 302 to that page; `AuthRecoveryLandingGuard` catches hash dumps. See `docs/operations/auth-redirects.md`. Do not change the mobile `norma://auth-callback` redirect.
...
*Social Content:*
- `cmo-generate` (6 hours) — Claude-generated brand tweets **only when there is a strong live/slate moment and non-stock media**. Wednesday-style empty slates (no live games, no near-term NFL/NCAAF, no recent alerts) skip rather than inserting `draft`+due `content_calendar` rows attached to catalog `game-detail-watch.png`. PR #40 pause-skip at publish time is unchanged.
