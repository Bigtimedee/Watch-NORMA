...
|---|---|---|---|
| A1 | Anonymous web visitor | none | `web/src/app/page.tsx`, `/demo`, `/status`, `/developers`, `/advertisers` (public CTA `/advertise` 301/308s here; no fabricated CTR / $0 proof chrome) |
...
4. **Account creation.** `app/(auth)/sign-up.tsx` supports email and password or Apple Sign In on iOS. Email signup requires verification. Returning users on `app/(auth)/sign-in.tsx` see Sign in with Apple first on iOS. If `signIn()` fails with invalid credentials, `mapSignInError` in `lib/auth-signin-errors.ts` shows an Apple CTA plus Forgot password / magic link (`/(auth)/forgot-password`). Recovery emails open `norma://auth-callback` (`app/auth-callback.tsx`); `PASSWORD_RECOVERY` stays on `/(auth)/reset-password` instead of bouncing to Games.
5. **Gate transition.** `AuthGate` in `app/_layout.tsx` watches `supabase.auth.onAuthStateChange`. On session acquisition while inside the `(auth)` group it calls `router.replace("/(tabs)/games")`, except during password recovery (`reset-password` / `auth-callback`).
...
| Connect PrizePicks / Underdog / Betr | `app/(tabs)/connections/pickem.tsx` | `connections` row, `provider_registry.category = dfs_pickem` |
| Scan a pick'em entry slip | `parse-bet-slip`, `ReviewScannedWagersSheet` | `wagers` with `market_type = player_prop`, `provider_key` prizepicks/underdog/betr |
...
`web/src/app/cmo/page.tsx` is the content calendar and approval surface. `cmo-generate` drafts brand social content with Claude **only when the slate is strong and media is non-stock** (thin Wednesday-style days skip; they do not insert `draft`+due rows with catalog `game-detail-watch.png`). `generate-social-content` and `generate-recap-content` produce post and recap variants, `publish-social-posts`, `cmo-publish`, and `cmo-publish-linkedin` distribute, and `fetch-social-metrics` reads performance back into `social_posts` and `social_hashtag_performance`. Consumer auto-post images are chosen by `_shared/social-media-select.ts` — settings / Tier-C sportsbook chrome is denylisted; seed-catalog game-detail / games-list is treated as stock and cannot auto-ship; Design/Expo Why Now captures are preferred. `cmo-publish` posts **twitter-only** `content_calendar` rows (`platform = 'twitter'`) to X; it must not tweet LinkedIn drafts (PR #32). It re-reads the row immediately before tweeting so a marketing `paused` (incident `310441ec`, 2026-09-15) cannot race into a post. `cmo-publish-linkedin` posts **linkedin-only** calendar rows to the NORMA company page (`https://www.linkedin.com/company/watch-norma/`, org `146336141`; `LINKEDIN_ORGANIZATION_ID` optional). The hourly `publish-social-posts` cron reads `social_posts` and routes by `post.platform` (x/instagram/facebook/tiktok/reddit — no linkedin). Instagram, Facebook, TikTok, and Reddit publishers are built but not fully wired to their platform APIs.
