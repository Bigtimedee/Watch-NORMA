...
1. **Football alert rules are written but unreachable.** `_shared/alert-scoring.ts` contains complete `checkMustNotify` branches for `nfl` and `ncaaf` (overtime, two minute one score, and more), and `evaluate-alerts/logic.ts` has football spread, total, moneyline, and close game evaluators. All of it is dead code: `evaluate-alerts/index.ts` line 103 defines `ALERTABLE_SPORTS = new Set(["ncaam", "nba", "mlb"])` and returns early with `reason: "football_alert_rules_not_yet_implemented"` for anything else. Meanwhile `lib/__tests__/sport-football-scaffold.test.ts` still asserts football evaluation is a no op. Shipping football means removing the guard and updating that test, together, deliberately.
2. **Football ingestion is partial.** ESPN base URLs for `nfl` and `ncaaf` exist in `poll-schedule` and `poll-boxscore`, but `lib/constants.ts` still defines `SPORTSDATAIO_BASE_URLS`, `SPORTRADAR_BASE_URLS`, and `ESPN_BASE_URLS` for `ncaam`, `nba`, and `mlb` only.
...
7. **Legacy API keys carry all four scopes.** `resolveApiKey()` defaults to the full scope set when `api_keys.scopes` is null. Any scope tightening must account for this.
