...
**23. Do not point hosted Auth `site_url` or web recovery `redirectTo` at the marketing homepage.**
On 2026-09-19, after the recovery email CTA was made clickable (PR #43), tapping the link still landed on `https://getnorma.app/?error=access_denied&error_code=otp_expired` with no set-password form. Production `site_url` was `https://getnorma.app` (marketing `/`) and `www.getnorma.app` was not allowlisted, so GoTrue rejected `redirectTo` and dumped users on `/`. **Required:** hosted `site_url` = `https://getnorma.app/auth/reset-password`; allowlist both apex and www exact reset/callback paths; web `resetPasswordForEmail` uses same-origin `/auth/reset-password` with no query string; mobile stays `norma://auth-callback`. Expired/invalid links must show “This reset link expired or was already used — request a new one” plus `/auth/forgot-password`. Never a silent marketing-home dump. Source of truth: `supabase/auth-redirects.json`, `docs/operations/auth-redirects.md`. Jest: `lib/__tests__/auth-recovery-landing.test.ts`.
