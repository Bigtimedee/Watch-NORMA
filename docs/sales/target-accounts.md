...
PrizePicks and Underdog are in `FANTASY_PLATFORMS` (`lib/fantasy-platforms.ts`), registered in `LSApplicationQueriesSchemes` (`app.json`), and seeded in `provider_registry` as `category = 'dfs_pickem'`. Platform selection is persisted on `follows.fantasy_source`. Both platforms have display names in `lib/constants.ts` `SPORTSBOOK_NAMES`. The `parse-bet-slip` Edge Function and `email-parser` recognize PrizePicks/Underdog entries. This is import/scan/email only — no live API connection exists. New native schemes (including `sleeper` / `yahoosports`) require an App Store binary; `app.json` is still 1.5.0.
