...
CREATE UNIQUE INDEX idx_ad_clicks_idempotency ON public.ad_clicks(idempotency_key) WHERE idempotency_key IS NOT NULL;
...
CREATE INDEX idx_webhook_endpoints_advertiser ON public.webhook_endpoints(advertiser_id);
...
CREATE INDEX idx_webhook_log_endpoint ON public.webhook_delivery_log(endpoint_id);
CREATE INDEX idx_webhook_log_next_retry ON public.webhook_delivery_log(next_retry_at)
