...
// #1/#2 — import { Modal } and import type { AgentConfig } moved to top; Modal merged into existing RN import; no duplicate imports
// #3    — state variable renamed from `interval`/`setInterval` → `pollInterval`/`setPollInterval` (was shadowing global setInterval)
// #4    — SliderRow and AgentSettingsSheet moved BEFORE export default (no declarations after export)
// #11   — useQueryClient removed (was imported and assigned but never used)
// #12   — handleToggle resets status: "idle" when deactivating

import { useState } from "react";
import {
  View, Text, Pressable, ScrollView, Switch, StyleSheet,
...
import type { AgentStatus, AgentConfig } from "../../../lib/types";
...
// ─── SliderRow — stepper row for numeric settings ────────────────────────────
...
function SliderRow({
  ...
  return (
    <View style={ss.row}>
      ...
      {/* Fixed: width-based bar instead of flex — avoids flex:0 overflow at min/max */}
      <View style={ss.trackBg}>
        <View style={[ss.trackFill, { width: `${fillPct}%` }]} />
      </View>
...
function AgentSettingsSheet({
  ...
  // Fixed: renamed from `interval`/`setInterval` — was shadowing the global setInterval timer API
  const [pollInterval, setPollInterval] = useState(config.poll_interval_seconds);
    ...
    <Modal visible animationType="slide" presentationStyle="pageSheet">
      <View style={ss.container}>
        ...
        <ScrollView style={ss.scroll} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <SliderRow label="Poll interval" value={pollInterval} min={30} max={120} step={30} unit="s" onChange={setPollInterval} />
        </ScrollView>
...
// ─── AgentScreen (main export) ────────────────────────────────────────────────

export default function AgentScreen() {
  // Fixed: removed useQueryClient — was imported and assigned but never used
  ...
  const handleToggle = async (value: boolean) => {
    ...
    // Fixed: reset status to "idle" when deactivating so status dot shows gray immediately
...
const ss = StyleSheet.create({
  ...
  // Fixed: width-based track instead of nested flex views (avoids flex:0 at extremes)
  trackBg: {
    height: 6, backgroundColor: "#334155", borderRadius: 3,
    overflow: "hidden", marginBottom: 10,
  },
  trackFill: { height: 6, backgroundColor: "#f97316", borderRadius: 3 },
...
const s = StyleSheet.create({
  ...
  setupBtn: { backgroundColor: "#f97316", borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12 },
  ...
  emptyCard: { backgroundColor: "#1e293b", borderRadius: 14, padding: 20, alignItems: "center" },
  ...
  alertTypeBadge: { borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3, alignSelf: "flex-start" },
